...
4. На агентском портале выберите роль **Агентство**, на клиентских — **Клиент**.
5. В кабинете агентства привяжите клиентские порталы (`PortalLink`) и укажите ID сделки сопровождения (`DealBinding`).
...
| CRUD | `/api/deal-bindings/` | Привязка сделки CRM к клиенту (agency) |
...
- При завершении задачи: system-строка в чате + Celery `post_task_complete_to_deal`:
  - пишет в таймлайн сделки: `Закрыта задача «…»: затрачено … . Остаток часов: N`
  - уменьшает поле **оставшихся часов** в сделке (поле **оплаченных** не меняется)
- Коды полей в `.env`: `BITRIX_DEAL_PAID_HOURS_FIELD`, `BITRIX_DEAL_REMAINING_HOURS_FIELD` (например `UF_CRM_…`)
- Если остаток пуст, а оплачено задано — при привязке сделки остаток инициализируется из оплаченных
